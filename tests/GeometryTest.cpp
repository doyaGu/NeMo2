...
TEST(VxIntersectTests, RayBox) {
    VxBbox box(VxVector(-1, -1, -1), VxVector(1, 1, 1));
    
    // Hit
    VxRay rayHit(VxVector(-5, 0, 0), VxVector(5, 0, 0));
    EXPECT_TRUE(VxIntersect::RayBox(rayHit, box));

    // Miss
    VxRay rayMiss(VxVector(-5, 5, 0), VxVector(5, 5, 0));
    EXPECT_FALSE(VxIntersect::RayBox(rayMiss, box));

    // Starts Inside
    VxRay rayInside(VxVector(0, 0, 0), VxVector(5, 0, 0));
    EXPECT_TRUE(VxIntersect::RayBox(rayInside, box));

    // Test overload with outputs
    VxVector inPoint, outPoint, inNormal, outNormal;
    EXPECT_TRUE(VxIntersect::RayBox(rayHit, box, inPoint, &outPoint, &inNormal, &outNormal));
    EXPECT_EQ(inPoint, VxVector(-1, 0, 0));
    EXPECT_EQ(outPoint, VxVector(1, 0, 0));
    EXPECT_EQ(inNormal, VxVector(-1, 0, 0));
    EXPECT_EQ(outNormal, VxVector(1, 0, 0));
}

TEST(VxIntersectTests, AABBAABB) {
    VxBbox boxA(VxVector(0,0,0), VxVector(2,2,2));
    
    // Overlapping
    VxBbox boxB(VxVector(1,1,1), VxVector(3,3,3));
    EXPECT_TRUE(VxIntersect::AABBAABB(boxA, boxB));
    
    // Touching
    VxBbox boxC(VxVector(2,0,0), VxVector(4,2,2));
    EXPECT_TRUE(VxIntersect::AABBAABB(boxA, boxC));

    // Separate
    VxBbox boxD(VxVector(3,0,0), VxVector(5,2,2));
    EXPECT_FALSE(VxIntersect::AABBAABB(boxA, boxD));
}

TEST(VxIntersectTests, OBBOBB) {
    VxMatrix identity = VxMatrix::Identity();
    VxBbox box(VxVector(-1,-1,-1), VxVector(1,1,1));
    VxOBB obb1(box, identity);
    
    // Intersect with self
    EXPECT_TRUE(VxIntersect::OBBOBB(obb1, obb1));
    
    // Intersect with translated
    VxMatrix mat_trans;
    mat_trans.SetIdentity();
    mat_trans[3][0] = 1.5f;
    VxOBB obb2(box, mat_trans);
    EXPECT_TRUE(VxIntersect::OBBOBB(obb1, obb2));
    
    // Separate
    mat_trans[3][0] = 2.5f;
    VxOBB obb3(box, mat_trans);
    EXPECT_FALSE(VxIntersect::OBBOBB(obb1, obb3));
    
    // Rotated intersection
    VxMatrix mat_rot;
    Vx3DMatrixFromEulerAngles(mat_rot, 0, 0, PI/4.0f);
    VxOBB obb4(box, mat_rot);
    EXPECT_TRUE(VxIntersect::OBBOBB(obb1, obb4));
}


TEST(VxIntersectTests, RayFace) {
    ...
    // Hit
    VxRay rayHit(VxVector(1,1,5), VxVector(1,1,-5));
    EXPECT_TRUE(VxIntersect::RayFace(rayHit, p0, p1, p2, norm, res, dist));
    EXPECT_NEAR(res.x, 1.0f, EPSILON);
    EXPECT_NEAR(res.y, 1.0f, EPSILON);
    EXPECT_NEAR(res.z, 0.0f, EPSILON);
    
    // Miss
    VxRay rayMiss(VxVector(6,6,5), VxVector(6,6,-5));
    EXPECT_FALSE(VxIntersect::RayFace(rayMiss, p0, p1, p2, norm, res, dist));

    // Backface culling
    VxRay rayHitBack(VxVector(1,1,-5), VxVector(1,1,5));
    EXPECT_TRUE(VxIntersect::RayFace(rayHitBack, p0, p1, p2, norm, res, dist)); // Should hit
    EXPECT_FALSE(VxIntersect::RayFaceCulled(rayHitBack, p0, p1, p2, norm, res, dist, i1, i2)); // Should be culled
}

TEST(VxIntersectTests, SphereAABB) {
    VxBbox box(VxVector(-1, -1, -1), VxVector(1, 1, 1));
