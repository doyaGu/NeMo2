...
static int ScalarConvertPixelBatch32(const XULONG* srcPixels, XULONG* dstPixels, int count, const VxPixelSimdConfig& config) {
    if (!config.enabled) {
        return 0;
    }
...
static int ScalarApplyAlphaBatch32(XULONG* pixels, int count, XBYTE alphaValue, XULONG alphaMask, XULONG alphaShift) {
    const int simdCount = count & ~3;
    if (simdCount <= 0) {
        return 0;
    }
...
static int ScalarApplyVariableAlphaBatch32(XULONG* pixels, const XBYTE* alphaValues, int count, XULONG alphaMask, XULONG alphaShift) {
    const int simdCount = count & ~3;
    if (simdCount <= 0) {
        return 0;
    }
...
TEST(SIMDDispatchConsistency, PixelOps_MatchScalarReference) {
    const VxSIMDDispatch* dispatch = VxGetSIMDDispatch();
    ASSERT_NE(dispatch, nullptr);
    ASSERT_NE(dispatch->Pixel.ConvertPixelBatch32, nullptr);
    ASSERT_NE(dispatch->Pixel.ApplyAlphaBatch32, nullptr);
    ASSERT_NE(dispatch->Pixel.ApplyVariableAlphaBatch32, nullptr);

    VxPixelSimdConfig cfg;
    cfg.enabled = true;
    cfg.alphaFill = true;
    cfg.alphaFillComponent = 0xFF000000u;

    // Simple ARGB->ARGB identity copy for RGB, keep alpha from alphaFill.
    cfg.channelCopy[0] = true;
    cfg.channelCopy[1] = true;
    cfg.channelCopy[2] = true;
    cfg.channelCopy[3] = false;

    cfg.srcMasks[0] = 0x00FF0000u; cfg.srcShiftRight[0] = 16; cfg.dstShiftLeft[0] = 16; cfg.dstMasks[0] = 0x00FF0000u;
    cfg.srcMasks[1] = 0x0000FF00u; cfg.srcShiftRight[1] = 8;  cfg.dstShiftLeft[1] = 8;  cfg.dstMasks[1] = 0x0000FF00u;
    cfg.srcMasks[2] = 0x000000FFu; cfg.srcShiftRight[2] = 0;  cfg.dstShiftLeft[2] = 0;  cfg.dstMasks[2] = 0x000000FFu;
    cfg.srcMasks[3] = 0xFF000000u; cfg.srcShiftRight[3] = 24; cfg.dstShiftLeft[3] = 24; cfg.dstMasks[3] = 0xFF000000u;

    std::vector<XULONG> src(17);
    std::vector<XULONG> dstS(17, 0x11223344u);
    std::vector<XULONG> dstV(17, 0x11223344u);
    for (int i = 0; i < static_cast<int>(src.size()); ++i) {
        src[i] = static_cast<XULONG>(0xA0000000u + (i * 0x00123456u));
    }

    const int rS = ScalarConvertPixelBatch32(src.data(), dstS.data(), static_cast<int>(src.size()), cfg);
    const int rV = dispatch->Pixel.ConvertPixelBatch32(src.data(), dstV.data(), static_cast<int>(src.size()), cfg);
    EXPECT_EQ(rS, rV);
    EXPECT_EQ(dstS, dstV);

    // Apply alpha
    std::vector<XULONG> pxS(17);
    std::vector<XULONG> pxV(17);
    for (int i = 0; i < static_cast<int>(pxS.size()); ++i) {
        pxS[i] = pxV[i] = static_cast<XULONG>(0x00ABCDEFu + (i * 3u));
    }
    const XULONG alphaMask = 0xFF000000u;
    const XULONG alphaShift = 24u;
    const XBYTE alpha = 0x7Fu;
    const int aS = ScalarApplyAlphaBatch32(pxS.data(), static_cast<int>(pxS.size()), alpha, alphaMask, alphaShift);
    const int aV = dispatch->Pixel.ApplyAlphaBatch32(pxV.data(), static_cast<int>(pxV.size()), alpha, alphaMask, alphaShift);
    EXPECT_EQ(aS, aV);
    EXPECT_EQ(pxS, pxV);

    // Apply variable alpha
    std::vector<XBYTE> alphas(17);
    for (int i = 0; i < static_cast<int>(alphas.size()); ++i) {
        alphas[i] = static_cast<XBYTE>(i * 9);
    }
    for (int i = 0; i < static_cast<int>(pxS.size()); ++i) {
        pxS[i] = pxV[i] = static_cast<XULONG>(0x00FEDCBAu + (i * 5u));
    }
    const int vaS = ScalarApplyVariableAlphaBatch32(pxS.data(), alphas.data(), static_cast<int>(pxS.size()), alphaMask, alphaShift);
    const int vaV = dispatch->Pixel.ApplyVariableAlphaBatch32(pxV.data(), alphas.data(), static_cast<int>(pxV.size()), alphaMask, alphaShift);
    EXPECT_EQ(vaS, vaV);
    EXPECT_EQ(pxS, pxV);
}
